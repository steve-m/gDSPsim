...
static gchar *mask[]=
{
  "11011110 ssssssss ttRR1000", // ADDSUB Tx, Smem, ACx
  "11101110 ssssssss ttRR110v", // ADDSUB Tx, dual(Lmem), ACx
};

static gchar *opcode[] = 
{
  "'ADDSUB' t,s,R", // ADDSUB Tx, Smem, ACx
  "'ADDSUB' t,'dual'(s),R", // ADDSUB Tx, dual(Lmem), ACx
};
...
static void execute(struct _PipeLine *pipeP, struct _Registers *Reg)
{
  ...
  switch ( pipeP->opcode_subType )
    ...
    case 0:
      ...
      // ACx_39_16 = Reg->DB - Tx
      // ACx_15_0 = Reg->DB + Tx
      alu(r, ALU_DB, R, ALU_DUAL_ADDSUB, Reg);
      return;

    case 1:
      // ADDSUB Tx, dual(Lmem), ACx
      R = (opcode.bop[2]>>4)&0x3;
      r = ((opcode.bop[2]>>6)&0x3)+4;
      // ACx_39_16 = Reg->DB - Tx
      // ACx_15_0  = Reg->CB + Tx
      alu(r, ALU_DBL, R, ALU_DUAL_ADDSUB, Reg);
      return;
    }
}
