...
void create_decode_window()
{
  ...
  gtk_widget_set_usize (GTK_WIDGET(dwn->decodeW), 500, 450);
...
#if 1
      if ( pipe[0] > 0 )
	gtk_clist_set_background(GTK_CLIST(dwn->clist),pipe[0],NULL);
...
void highlight_pipeline(WordA follow)
{
  ...
  if ( follow )
      ...
      if ( (dwn->start <= follow) && (follow <= dwn->end) )
	{
	  ...
	  gtk_clist_set_background(GTK_CLIST(dwn->clist),row,
				   &pipe_color[0]);
	  pipe[0]=row;
	}
    }
