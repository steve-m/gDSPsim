...
static gchar *opcode[] = { "SUBC a,s" };
static gchar *comment[]= { "$(s)=$(s)+ $(a) + C" };

/* This definition is global because another routine will make have
 * an array that points to all the different instruction classes.
 */
Instruction_Class SUBC_Obj =
{
  "SUBC",
  NULL, // prefetch
  NULL, // fetch
...
static void execute(struct _PipeLine *pipeP, struct _Registers *Reg)
{
  ...
  shifter(2,Reg,2,15,4);

  // Save a copy of Register
  if ( regNum )
    reg_union.gp_reg = MMR->B;
  else
    reg_union.gp_reg = MMR->A;
  
  alu(1,regNum,regNum,1,Reg);
  ...
  if ( reg_union.gint64 >= 0 )
	...
	reg_union.gp_reg = MMR->A;
      ...
      reg_union.gint64 = (reg_union.gint64 << 1) + 1;
    }
  else
    {
