...
static gchar *mask[]=    { "0101010s aaaaaaaa" };
static gchar *opcode[] = { "DSUB a,s" };
static gchar *comment[]= { "$(s)=$(s)-$(a)" };

Instruction_Class DSUB_Obj =
{
  "DSUB",
  NULL, // prefetch
  NULL, // fetch
...
static void execute(struct _PipeLine *pipeP, struct _Registers *Reg)
{
  if (pipeP->word_number == 1 )
    {
      alu( 1, (pipeP->current_opcode & 0x100 )>>8,(pipeP->current_opcode & 0x100 )>>8, 9, Reg );
    }
}
