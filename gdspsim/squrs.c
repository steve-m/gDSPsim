...
static gchar *mask[]=  { "0011101d aaaaaaaa"  };
static gchar *opcode[] = { "SQURS a,d" };
static gchar *comment[]= { "$(d)= $(d) - $(a) * $(a); T=$(a)" };

/* This definition is global because another routine will make have
 * an array that points to all the different instruction classes.
 */
Instruction_Class SQURS_Obj =
{
  "SQURS",
  NULL, // prefetch
  NULL, // fetch
  ...
  if ( pipeP->word_number == 1)
    {
      ...
      // Accumulate by negative d (2+d)
      // Store in A or B (d)
      MMR->T = Reg->DB;
      d = ( pipeP->current_opcode & 0x100 ) > 8;
      
      multiplier(1,1,2+d,d,Reg);
    }
}
