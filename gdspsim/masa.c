...
static void execute(struct _PipeLine *pipeP, struct _Registers *Reg)
{
  if ( pipeP->opcode_subType == 0 )
    {
      // X operand is from A[32-16] register (2)
      // Y operand is from DB register (1)
      // Accumulate by subtracting from B (4)
      // Store B
      
      multiplier(2,1,4,1,Reg);
  ...
  else
    {
      ...
      // X operand is from T registers (0)
      // Y operand is from A[32-16] register (2)
