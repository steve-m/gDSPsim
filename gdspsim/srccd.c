...
static gchar *mask[]=    { "10011101 xxxxcccc" };
static gchar *opcode[] = { "SRCCD h,c" };
static gchar *comment[]= { "if $(c) then $(x)=BRC" };

/* This definition is global because another routine will make have
 * an array that points to all the different instruction classes.
 */
Instruction_Class SRCCD_Obj =
{
  "SRCCD",
  NULL, // prefetch
  NULL, // fetch
...
static void execute(struct _PipeLine *pipeP, struct _Registers *Reg)
{
  if ( check_condition( (pipeP->current_opcode & 0xf) | 0x40) )
    {
      write_data_mem(Reg->EAB,MMR->BRC);
    }
  else
    {
